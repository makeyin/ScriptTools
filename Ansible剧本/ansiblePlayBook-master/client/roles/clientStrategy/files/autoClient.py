...
TOSEAL_LIMIT2 = 70*1024
TOSEAL_LIMIT3 = 80*1024
...
blockLaggingMinersDict = {}  #ip:nodeBlockHeight
blockLaggingMinersList = []  #[(ip:nodeBlockHeight)]
senderDict = {}
ipfsBlockHeightAVG = 0
ipfsBlockHeightLen = 0
...
def getMinerInfo(file_name):
    #print(file_name)
    global recipientMostList,recipientMoreList,recipientMuchList,blockLaggingMinersList,blockLaggingIPList,senderAvailableList,ipfsBlockHeightAVG,ipfsBlockHeightLen
    ...
    #sender按照power值大小排序，由小到大，越空闲越当client
    senderList = sorted(senderDict.items(), key=lambda d:d[1], reverse = False)
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," NOTICE: sender List order by power asc: ",senderList)
    #限制发送算力的client个数
    if(len(senderList) > Max_Sender_Num):
        senderList = senderList[:Max_Sender_Num]
    #print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," NOTICE: Max_Sender_Num is : ",Max_Sender_Num)

    if( ipfsBlockHeightAVG != 0 and ipfsBlockHeightLen != 0):
        ipfsBlockHeightAVG = ipfsBlockHeightAVG/ipfsBlockHeightLen
    
    #区块落后的IP保存在blockLaggingIPList
    blockLaggingMinersList = sorted(blockLaggingMinersDict.items(), key=lambda d:d[1], reverse = True)

    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," NOTICE: ipfs block height AVG : ",ipfsBlockHeightAVG)
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," NOTICE: our node height block height : ",blockLaggingMinersList[0][1])
    if(ipfsBlockHeightAVG > blockLaggingMinersList[0][1]):
        lagginIDX = [i for i,x in enumerate(blockLaggingMinersList) if x[1] < blockLaggingMinersList[0][1]-blockLaggingThreshhold]
    else:
        lagginIDX = [i for i,x in enumerate(blockLaggingMinersList) if x[1] < ipfsBlockHeightAVG-blockLaggingThreshhold]
    ...
    # recipientMoreDict = {} #ip:toSeal
    # recipientMuchDict = {} #ip:toSeal
    # blockLaggingMinersDict = {}

def groupRecepient(strList):
    global recipientMostDict,recipientMoreDict,recipientMuchDict,blockLaggingMinersDict,senderDict,ipfsBlockHeightAVG,ipfsBlockHeightLen
    ...
    if( miner.ipfsBlockHeight != 0 ):
        ipfsBlockHeightLen = ipfsBlockHeightLen + 1
        ipfsBlockHeightAVG = ipfsBlockHeightAVG + miner.ipfsBlockHeight
    ...
    blockLaggingMinersDict[miner.ip] = miner.nodeBlockHeight
    senderDict[miner.ip] = miner.power
...
def writeSenderRecipientMappingFile(inputFile,outputFile,mapping):
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," inputFile: ",inputFile)
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," outputFile: ",outputFile)
    hostInfoDict = {}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mostSenderTimes', type=int, default = 5)
    parser.add_argument('--moreSenderTimes', type=int, default = 2)
    parser.add_argument('--muchSenderTimes', type=int, default = 1)
    parser.add_argument('--blockLaggingThreshhold', type=int, default = 10)
    parser.add_argument('--Max_Sender_Num', type=int, default = 10)
    parser.add_argument('--TOSEAL_LIMIT3', type=int, default = 81920)
    ...
    playBookHostFile = args.hostfile
    # 放在与playbook同一级目录
    if("" ==  playBookHostFile):
        fatherPath= os.path.dirname(os.path.realpath(__file__))+"/../../.."
        playBookHostFile = fatherPath + "/hosts"

    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," Params: Max_Sender_Num ",Max_Sender_Num,", blockLaggingThreshhold ",blockLaggingThreshhold," , mostSenderTimes ",mostSenderTimes, " , hostfile ", playBookHostFile )
