...
def checkPeersConnection():
    ...
    ### master实际连上的slave
    actualSlavePeersList = os.popen("~/bin/lotus-miner net peers | awk -F',' '{print $1}' ").read().rstrip().split("\n")
    ### 真实部署的slave
...
if __name__ == '__main__':
  ...
  if(daemonN!=-1):
    if(daemonN==0 or minerN==0):
      ...
      #sendDingdingMessage("机器:%s 严重警告: lotus 进程挂了!!! daemon 进程���量: %s. miner 进程数量: %s "%(hostname,daemonN,minerN))
  ...
  minerAvailable=-1;walletBalance=-1;postBalance=-1;
  wallet_health = True; available_diff_health = True
  minerNum = ""; sectorsStat=""; power="";powerStatus="";walletStatus=""
  postWallet="";postWalletStatus="";postWallet_health = True;
  ### miner进程存在时才可以检查miner信息
  if(minerN!=0):
    walletBalance,minerAvailable,minerNum,power = checkMinerInfo()
    sectorsStat = "扇区状态:\n" + checkSectorStatus()
    powerStatus = "当前算力: " + power
    walletStatus = "当前worker钱包余额: " + str(walletBalance) + ", 矿工余额: " + str(minerAvailable)
    postWallet,postBalance=checkPostWallet()
    postWalletStatus = "当前post钱包余额: " + str(postBalance)

  ### post wallet余额小于walletBalanceMinimum就报警
  if(postBalance != -1 and int(postBalance) < walletBalanceMinimum-100):
    postWallet_health = False
  ...
  if(minerAvailable != -1 and minerAvailable < availableDifference):
  ...
  if(process_health and chain_health and wallet_health and available_diff_health and nonce_health):
    ...
    if(not wallet_health):
      report += "\n严重警告: worker wallet钱包余额不足,请尽快接水龙头!!! 当前余额为 %s"%(walletBalance)
    if(not postWallet_health):
      report += "\n严重警告: post wallet钱包余额不足,请尽快处理!!! 当前余额为 %s"%(walletBalance)
    if(not available_diff_health):
      report += "\n严重警告: 矿工账户抵押物不足!!!矿工账户available为 %s "%(minerAvailable)
    if(not nonce_health):
      report += "\n严重警告: 矿工消息池Nonce需要fix!!! 本地消息池消息数量 past: %s, cur: %s, future: %s"%(pastMsg,curMsg,futureMsg)
  sendDingdingMessage(report + "\n" + powerStatus + "\n" + sectorsStat + "\n" +msgStatus + "\n" + walletStatus  + "\n" + postWalletStatus,defaultToken)
