...
function setParam(){
  ...
  if [[ `cat /proc/cpuinfo | grep "model name" | grep AMD` ]] && [[  `df -l | grep tank4` ]]; then
      ...
      export OPTION22=/tank4/vde/                  #跟OPTION15配合,新增加的nvme的路径,如果有nvme的话路径是/tank4/vde,没有新增nvme的话填写None
...
case "$1" in
 ...
 wp|startwindow)
  ...
  deploy)
        ...
        ;;
  *)
        echo "Usage: ~/bin/view_lotus.sh {start|stopall|mining|slave|restart|restartSlave|cleanEnv|log|logm|logs|id|peers|logwd|cleanwindow|startwindow|pwd|pinfo}" || true
        exit 1
esac
exit 0