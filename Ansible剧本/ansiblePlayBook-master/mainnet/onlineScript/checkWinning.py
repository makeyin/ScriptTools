##### python3 checkWinning.py --hour=3
...
def getOnChainInfo(minerId,startTime,endTime):
  ...
  chainCmd=""" ~/bin/lotus chain list --count=%s | awk -F'[()]' '{if($2>"%s" && $2<"%s")print $0}' > %s """%(int(expectedNetBlockNum+10),startTime_f,endTime_f,chainListFileTmp)
...
def getLogWithTime():
  ...
  mlogFileWithinTimeCmd="""grep '^2020' --text /home/$USER/log/%s | egrep -a "mbi info timeout|mined block in the past|distribute win task|getBaseInfo finished took|IsComputeWin|mining delay|mined new block|mineOneSep|loop again|failed getting beacon entry|received wtask|attempting to mine a block" | awk '{if($1 > "%s" && $1 < "%s"){print $0}}' > %s"""%(mlogFile,startTime,endTime,mineLogFile)
...
def printNumpyRes(alist,name):
  ...
  print("%-16s   %10s   %10s  %10s   %10s   %10s  %10s   %10s   %10s  %10s  %10s  %10s  %10s  %10s  %10s"%(name,avg,std,minV,v10,v50,v60,v70,v80,v90,v95,v97,v98,v99,maxV))
...
def analyzeSelfMinedBlock():
  ...
  getBeaconFailCount=os.popen("grep 'failed getting beacon entry' %s | grep 310853 | wc -l"%mineLogFile).read().rstrip()
  winnerTrueCount=os.popen(""" grep IsComputeWin %s | awk -F'Count":' '{print $NF}' | awk -F'}' '{print $1}' | grep -v 0 | wc -l """%mineLogFile).read().rstrip()
  winnerFalseCount=os.popen(""" grep IsComputeWin %s | awk -F'Count":' '{print $NF}' | awk -F'}' '{print $1}' | grep 0 | wc -l """%mineLogFile).read().rstrip()
  ...
  sendToPosterFailedCount=os.popen(""" grep "received wtask timeout" %s | grep "sents: 0"| wc -l """%mineLogFile).read().rstrip()
  receiveSlaveFailedCount=os.popen(""" grep "received wtask timeout" %s | grep "received: 0"| grep "sents: 1" | wc -l """%mineLogFile).read().rstrip()
  ...
  print("    master刮中奖的次数: ",winnerTrueCount,"        未刮中奖的次数: ",winnerFalseCount,"       mbi超时: ",mbiTimeoutCount)
  print("   收到poster返回的次数: ",receiveSlaveSuccessCount,"     发送给poster失败: ",sendToPosterFailedCount,"     未收到poster返回: ",receiveSlaveFailedCount)
  ...
  if(lostBlockList == []):
    ...
    for cid in lostBlockList:
      ...
      parentsList = os.popen(""" ~/bin/lotus chain getblock %s | jq '.Parents[]|."/"' """%cid).read().rstrip().replace('"','').split("\n")
...
if __name__ == '__main__':
  ...
  logFile = os.popen("ls -l /home/$USER/log | grep  lotus | grep -v _withTime |tail -n 1 | awk '{print $9}'").read().rstrip()
  #mlogFile = "storage-miner.05181838.out"
  mlogFile = os.popen("ls -l /home/$USER/log | grep  lotus-winning | grep -v _withTime |tail -n 1 | awk '{print $9}'").read().rstrip()
  block_time = 30  #s
  ...
  if(mineOneSepTotal_TimeList == []):
    ...
    print ("%-16s   %10s   %10s   %10s   %10s   %10s  %10s  %10s  %10s  %10s  %10s   %10s   %10s  %10s  %10s"%("[module]","[avg]","[std]","[min]","[10th]","[50th]","[60th]","[70th]","[80th]","[90th]","[95th]","[97th]","[98th]","[99th]","[max]"))
