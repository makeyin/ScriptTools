...
genesisPath="/home/devnet/genesis-sectors"
templateFile="/home/devnet/genesis-sectors/localnet.json"
...
setsid ~/bin/lotus daemon --lotus-make-genesis="/home/devnet/bin/devnet.car" --genesis-template=${templateFile} --bootstrap=false > ~/log/lotus.`date +"%m%d%H%M"`.out 2>&1 &
sleep 15
...
### init miner
~/bin/lotus-miner init --genesis-miner --actor=t01000 --sector-size=${SECTOR_SIZE} --pre-sealed-sectors=${genesisPath} --pre-sealed-metadata=${genesisPath}/pre-seal-t01000.json --nosync
### start miner
sed -i 's/MinerMode =.*/MinerMode = "normal"/g' ~/.lotusminer/config.toml
setsid ~/bin/lotus-miner run --nosync > ~/log/lotusminer.`date +"%m%d%H%M"`.out 2>&1 &

kill `ps -u $USER -f | grep "python" | grep 8989 | grep -v grep | awk '{print $2}'` 2> /dev/null
cd ~/bin; nohup python3 -m http.server 8989 &
kill `ps -u $USER -f | grep "lotus-fountain" | grep -v grep | awk '{print $2}'` 2> /dev/null
setsid ~/bin/lotus-fountain --repo=~/.lotus run --from=${walletDefault} --front=0.0.0.0:9292 > ~/log/foutain.`date +"%m%d%H%M"`.out 2>&1 &