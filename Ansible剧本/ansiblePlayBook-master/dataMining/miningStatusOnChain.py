...
def analyzeBlockRatio(minerIdList,startTime,endTime):
  ...
  expectedBlockNum=(float(struct_endTime)-float(struct_startTime))/45
  ###实际出块数量
  ...
  print (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),"统计时间为 %s 至  %s , 期间全网预期出块个数 %s, 实际出块个数 %s"%(startTime,endTime,int(expectedBlockNum),totalBlockNum))
  print (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),"############################################################")
  ...
  for minerId in minerIdList:
    ...
    if( int(minerInfoDict["blockNum"]) == 0 ):
      ...
      minerInfoDict["transRate"]=format(float(minerInfoDict["actualRate"])*100/float(minerInfoDict["expectRate"]), '.2f')
...
def printAllMinerInfo():
  ...
  for i in afterSortList:
    ...
    print("%-10s   %10s   %10s%%  %10s     %10s%%      %10s%%     %10s%%"%(minerId,v["power"],v["powerRate"],v["blockNum"],v["expectRate"],v["actualRate"],v["transRate"]))
...
if __name__ == '__main__':
  ...
  #minerIdList=["t018836","t018985"]
  minerIdList=["t016970","t017504","t018985","t01210","t09999","t017539","t018836","t01031","t016969","t02932","t019125","t02112"]
  startTime="2020-02-25T18:00:00"
  endTime="2020-02-25T22:00:00"
  #######################################
