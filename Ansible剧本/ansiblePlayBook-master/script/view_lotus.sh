...
function start(){
  ...
  kill -9 `ps -u $USER -f | grep "lotus daemon" | grep -v grep | grep -v view_lotus.sh| awk '{print $2}'` 2> /dev/null
  stopTryNum=1
  while [ -n "`ps aux | grep lotus | grep -v grep | grep -v view_lotus.sh`" -a $stopTryNum -lt 20 ]
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus dying out,  try num: $stopTryNum"
      let stopTryNum++
      sleep 5
  done
  mkdir -p ~/log
  ...
  #export LOTUS_PROMETHEUS=/ip4/0.0.0.0/tcp/15400
  ...
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus daemon up, try num: $startTryNum"
      let startTryNum++
      sleep 10
      ~/bin/lotus net id
  done
...
function startMiner(){
  ...
  stopTryNum=1
  while [ -n "`ps aux | grep lotus-storage-miner | grep -v grep | grep -v view_lotus.sh`" -a $stopTryNum -lt 20 ]
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus-storage-miner dying out,  try num: $stopTryNum"
      let stopTryNum++
      sleep 5
  done
  ...
  #export STORAGE_PROMETHEUS=/ip4/0.0.0.0/tcp/15401
  export VDE_SWITCH=true
  ...
  while [ "$?" != "0" -a $startTryNum -lt 20 ]
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus-storage-miner daemon up, try num: $startTryNum"
      let startTryNum++
      sleep 10
      ~/bin/lotus-storage-miner net id
  done
...
function startSlave(){
  ...
  stopTryNum=1
  while [ -n "`ps aux | grep lotus-slave-miner | grep -v grep | grep -v view_lotus.sh`" -a $stopTryNum -lt 20 ]
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus-slave-miner dying out,  try num: $stopTryNum"
      let stopTryNum++
      sleep 5
  done
  ...
  export RUST_LOG=info
  export VDE_SWITCH=true
  #export SLAVE_PROMETHEUS=/ip4/0.0.0.0/tcp/15402
  export VDE_SWITCH=true
  ...
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus-slave-miner daemon up, try num: $startTryNum"
      let startTryNum++
      sleep 5
      ~/bin/lotus-slave-miner net id
  done
...
function startPoster(){
  ...
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus-poster dying out,  try num: $stopTryNum"
      let stopTryNum++
      sleep 5
  done
  ...
  while [ "$?" != "0" -a $startTryNum -lt 10 ]
  do
      echo `date +'%Y-%m-%d %H:%M:%S'` "waiting for lotus-poster daemon up, try num: $startTryNum"
      let startTryNum++
      sleep 5
      ~/bin/lotus-poster net id
  done
...
function guard(){
  ...
  echo "stop old lotusGuard.sh"
  kill -9 `ps -u $USER -f | grep lotusGuard.sh | grep -v grep | awk '{print $2}'` 2> /dev/null
  echo "start lotusGuard.sh"
  setsid ~/bin/lotusGuard.sh $USER > ~/log/lotusGuard.out 2>&1  &
}
