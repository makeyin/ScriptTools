...
function start(){
  ...
  export GPU_HASH=true  ## 开GPU做默克树和CS
  export GPU_BELL=true  ## 开GPU做零知识证明
  export GPU_NUM=4   ## 显卡数量，默认4
  export GPU_MEM=9700    ## 扣除POST会占用显存外每块显卡可用于做默克树和seal的显存大小，默认9700，适合4G4U挖1g，如果是32G需要手工改为8800，如果显存大于11000M显卡需按照情况调整
  export VDE_SWITCH=true  ## 默认设置为true
  export ZK_NUM=2    ## Seal零知识证明并行个数（每个sector的并行零知识证明数），默认1，4G4U建议设成2，劳斯莱斯建议设成5
  #export FastStaging=false
...
function startMiner(){
  ...
  stopTryNum=1
  while [ -n "`ps aux | grep lotus-storage-miner | grep -v grep | grep -v view_lotus.sh`" -a $stopTryNum -lt 10 ]
  do
...
function startSlave(){
  ...
  export GPU_HASH=true  ## 开GPU做默克树和CS
  export GPU_BELL=true  ## 开GPU做零知识证明
  export GPU_NUM=4   ## 显卡数量，默认４
  export GPU_MEM=10500    ## 扣除POST会占用显存外每块显卡可用于做默克树和seal的显存大小，默认9700，适合4G4U挖1g，如果是32G需要手工改为8800，如果显存大于11000M显卡需按照情况调整.如果是slave设置为10000
  export VDE_SWITCH=true  ## 默认设置为true
  export ZK_NUM=2    ## Seal零知识证明并行个数（每个sector的并行零知识证明数），默认1，4G4U建议设成2，劳斯莱斯建议设成5
