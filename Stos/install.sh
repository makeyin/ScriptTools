...
cat >/root/stos/rsnode${i}/installSds.sh << EOF
#!/usr/bin/expect
spawn /root/stos/rsnode${i}/ppd config -w -p
expect "password"
send "Night123\n"
expect "again"
send "Night123\n"
...
chmod +x $sdsHome/ppd
cd $sdsHome
nohup $sdsHome/installSds.sh  >> install.log 2>&1  &
sleep 5s
neiwang=`ip a | grep inet | grep -v inet6 | grep -v 127 | sed 's/^[ \t]*//g' | cut -d ' ' -f2 | awk -F '/' '{print $1}' | head -n 1|awk -F '.' '{print $4}'`
double=$(( 300 *  $i ))
Netw=`expr ${neiwang} + 18081 + ${double}`
